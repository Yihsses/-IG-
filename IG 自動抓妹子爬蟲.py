...
def get_image(path,url):
    ...
    if res.status_code == 200:
        ...
        if (predict_gender(path) != "Female") :
            remove_img("./girl",image)
            return
...
if __name__ == '__main__':
    ...
    driver.get('https://www.instagram.com/good_looking.tw/') 
    path = 0
    while(1):
        try:
            ...
            try :
                    ...
                    lis = []
                    for image in images:
                        if (image.get_attribute('src')[0] == "h"):
                            print(image.get_attribute('src'))
                            s = image.get_attribute('src').split("/")
                            s = s[5].split("_")
                            get_image(s[0],image.get_attribute('src'))
                            path +=1
                          
                            lis.append(image.get_attribute('src'))
